...
public class GenreDaoTest {
    ...
    public void insertAndDeleteGenre() {
        ...
        assertNotEquals("Ambient", result2.get(0).name);
    }
    ...
    public void insertAndModifyGenre() {
        ...
        genreDao.update(new Genre(1, "Test2", 1));
    ...
    @Test
    public void insertAndGetGenreWithService() {
        Context context = ApplicationProvider.getApplicationContext();
        for (Map<String, Object> fakeMedia : fakeMediaStoreData){
            long idGenre = genreService.addGenre(fakeMedia.get(MediaStore.Audio.Media.GENRE).toString(), statisticService, context);
            Log.d(fakeMedia.get(MediaStore.Audio.Media.TITLE).toString(), String.valueOf(idGenre));
        }

        try {
            List<Genre> result1 = LiveDataTestUtil.getOrAwaitValue(genreDao.getAllLive());
            for (Genre genre : result1){
                Log.d("TestGenreName", genre.name);
            }
        ...
        assertEquals(1, result2.get(0).id);
        assertEquals("Afrobeat", result2.get(0).name);
        assertEquals("J-rock", result2.get(6).name);
    }
}
